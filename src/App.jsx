...
import MLCallbackView from './views/MLCallbackView';
...
function App() {
  ...
  // Initial Path Detection for SPA Routing
  React.useEffect(() => {
    const path = window.location.pathname;
    if (path.includes('/auth/mercadolivre/callback')) {
      dispatch({ type: 'SET_VIEW', payload: 'auth-callback' });
    }
  }, [dispatch]);
  ...
  const renderView = () => {
    switch (state.view) {
      ...
      case 'auth-callback': return <MLCallbackView />;
